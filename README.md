# 🎓 Student Performance Prediction System (EduPredict)

## 📊 Overview
A Django-based web application that uses Machine Learning to predict student performance in online courses. The system helps educators identify at-risk students early and implement timely interventions.

## ✨ Features
- **Predictive Analytics**: Multiple ML algorithms for performance prediction
- **Interactive Dashboard**: Real-time analytics and visualizations
- **Student Management**: Complete CRUD operations
- **CSV Import**: Bulk upload student data
- **Role-Based Access**: Separate interfaces for instructors/students
- **PDF Reporting**: Export predictions as professional reports
- **Data Visualization**: 7+ interactive chart types

## 🛠️ Tech Stack
- **Backend**: Django, Python
- **Frontend**: HTML, CSS, JavaScript, Bootstrap 5
- **ML Libraries**: Scikit-learn, Pandas, NumPy
- **Visualization**: Matplotlib, Chart.js
- **Reporting**: ReportLab


Open browser: http://localhost:8000
